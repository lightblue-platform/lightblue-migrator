...
if [ $1"x" == "x" ] || [ $2"x" == "x" ] || [ $3"x" == "x" ] || [ $4"x" == "x" ] || [ $5"x" == "x" ] || [ $6"x" == "x" ]; then
  echo "Usage: ./yearlyMigrationJobs.sh <entityName> <startDate> <endDate> <frequency> <expectedExecutionTime> <createdBy> [<startingId>] [<yearlyIdIncrement or use the last one>]"
  echo "Example: ./yearlyMigrationJobs.sh user 2014-01-01 2014-12-31 1 30000 derek63 0 "
...
while true; do
  ...
  if [ "${INCREMENT_WITH_LAST_ID}x" == "x" ]; then
      INCREMENT_ID=$(cat "/tmp/last_job_id")
  fi
  i=$((1+INCREMENT_ID))
